...
describe('Continents Components', () => {
  const countryDetail = {
    code: 'AR',
    emoji: MOCK_COUNTRY.emoji,
    name: MOCK_COUNTRY.name,
    continent: { code: CONTINENTS_CODES.SA }
  }

  beforeEach(() => {
    const mockClient = createMockClient()

    mockClient.setRequestHandler(CONTINENT_GAME_DATA_QUERY, () =>
      Promise.resolve({
        data: { continents: MOCK_CONTINENTS, countries: MOCK_CONTINENTS_COUNTRIES }
      })
    )
  ...
  it('ContinentCard - is rendering properly', () => {
    const wrapper = mount(ContinentCard, {
      props: { data: countryDetail, showName: false, continentCode: 'ALL' }
    })
