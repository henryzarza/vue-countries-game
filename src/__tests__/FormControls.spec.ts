...
describe('Form Controls', () => {
  it('FormError - is showing the errors properly', () => {
    const wrapper = mount(FormError, { props: { errors: [{"$propertyPath":"email","$property":"email","$validator":"email","$uid":"email-email","$message":"Email format is invalid","$params":{"type":"email"},"$response":false,"$pending":false},{"$propertyPath":"email","$property":"email","$validator":"minLength","$uid":"email-minLength","$message":"Email must be at least 6 characters","$params":{"min":6,"type":"minLength"},"$response":false,"$pending":false}] } })
  ...
  it.each([
    ['FormInput', 'input', FormInput],
    ['FormTextarea', 'textarea', FormTextarea]
  ])('%s - label is showing * and has the error class when the field is required', (_, element, Component) => {
    const wrapper = mount(Component, {
      props: {
        nameId: "test",
        label: "Test",
