...
<template>
  <div v-if="code" class="fixed opacity-80 bg-black inset-0 z-10 transition-opacity" />
  <section
    class="flex flex-col fixed -bottom-1 left-0 right-0 bg-slate-100 dark:bg-slate-800 w-[30rem] max-w-full mx-auto rounded border-2 border-slate-300 transition-transform z-20 dark:border-slate-700"
  ...
  >
    <button
      aria-label="close"
      class="absolute -top-12 right-4 py-2 px-4 text-2xl bg-slate-200 dark:bg-slate-900 text-slate-900 transition-all hover:bg-violet-200 hover:text-violet-700 font-extrabold rounded delay-200 dark:text-slate-50 dark:hover:bg-teal-100 dark:hover:text-teal-700 border-2 border-slate-300 z-20 dark:border-slate-700 border-b-0"
    ...
    <!-- Empty data or error -->
    <div
      v-else-if="!result?.country || error"
    ...
    <template v-else>
      <header
        class="bg-slate-200 dark:bg-slate-900 px-4 pb-4 border-b-2 flex flex-col gap-1 border-b-slate-300 dark:border-b-slate-700"
      >
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4">
        <div class="flex flex-col gap-1">
          <span class="text-sm font-semibold text-zinc-950 dark:text-slate-50"> Currencies </span>
        ...
        <div class="flex flex-col gapp-1">
          <span class="text-sm font-semibold text-zinc-950 dark:text-slate-50"> Indicator </span>
          <span
            class="text-base text-zinc-950 dark:text-slate-50 font-bold"
        ...
        <div class="flex flex-col">
          <span class="text-sm font-semibold text-zinc-950 dark:text-slate-50"> Continent </span>
        ...
        <div class="flex flex-col gap-1 col-span-2">
          <span class="text-sm font-semibold text-zinc-950 dark:text-slate-50"> Languages </span>
          <span
          ...
          >
            ...
            <strong class="font-semibold">{{ language.code }}</strong>
          </span>
        </div>
        <div class="flex flex-col">
          <span class="text-sm font-semibold text-zinc-950 dark:text-slate-50"> AWS Region </span>
      ...
      <template v-if="result.country.states && result.country.states.length > 0">
        <h6 class="px-4 text-lg text-zinc-950 dark:text-slate-50 font-semibold mb-2">States</h6>
        <ul
          class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 px-4 pb-4 max-h-72 overflow-y-auto"
